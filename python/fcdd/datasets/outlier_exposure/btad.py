        ...
        if limit_per_anomaly and limit_var is not None:
            ...
            for l in set(self.anomaly_labels.tolist()):
                ...
                if limit_var < len(linclsses):
                    ...
                    self.logprint(
                        'OEBTAD shall be limited to {} samples per anomaly label, '
                        'but BTAD anomaly label {} contains only {} samples, thus using all.'
            ...
            if limit_var is not None and limit_var > len(self):
                self.logprint(
                    'OEBTAD shall be limited to {} samples, but BTAD contains only {} samples, thus using all.'
