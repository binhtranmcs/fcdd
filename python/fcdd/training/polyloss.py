...
class PLTrainer(BaseADTrainer):
    def loss(self, outs: Tensor, ins: Tensor, labels: Tensor, gtmaps: Tensor = None, reduce='mean'):
        ...
        if self.objective in ['poly']:
            loss = self.__poly_loss(outs, ins, labels, gtmaps, reduce)
        else:
            raise NotImplementedError('Objective {} is not defined yet.'.format(self.objective))
        return loss

    def __poly_loss(self, outs: Tensor, ins: Tensor, labels: Tensor, gtmaps: Tensor, reduce: str):
        loss = outs ** 2
    ...
    def __supervised_loss(self, loss: Tensor, labels: Tensor):
        if self.net.training:
            ...
            norm = (-loss[labels == 0]).exp() + 1e-31
            norm = -norm.log() + EPS * (1 - norm)
            anom = (1 - (-loss[labels == 1]).exp()) + 1e-31
            anom = -anom.log() + EPS * (1 - anom)
